...
func (s *JenisPendapatanService) IsKodeExist(kode string, id uint) error {
	var count int64
	query := s.db.Model(&models.JenisPendapatan{}).Where("kode=?", kode)
	if id > 0 {
		query.Where("id !=?", id)
	}
	query.Count(&count)
	if count > 0 {
		return fmt.Errorf("kode jenis pendapatan sudah digunakan")
	}
	return nil
}
